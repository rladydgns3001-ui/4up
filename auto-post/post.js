  ...
  try {
    ...
    if (jsonMatch) {
      ...
      article.content = article.content
        // 본문에서 h1 태그 완전 제거 (워드프레스 제목이 h1이므로 중복 방지)
        .replace(/<h1[^>]*>.*?<\/h1>/gi, '')
        // # 제목 형식도 제거 (마크다운 h1)
        .replace(/^# (.+)$/gm, '')
        // ## 제목 → <h2>제목</h2>
      ...
      if (imageHtml1) {
        ...
        article.content = article.content.replace("[IMAGE_PLACEHOLDER_1]", "");
      }
      if (imageHtml2) {
        article.content = article.content.replace("[IMAGE_PLACEHOLDER_2]", imageHtml2);
      } else {
